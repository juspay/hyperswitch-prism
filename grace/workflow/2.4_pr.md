...
### 3d: Automated secret scan (mandatory)

Visual diff inspection misses base64-encoded secrets, JWTs, and multi-line encoded
credentials. Run an automated scanner:

```bash
# Option 1: gitleaks (preferred)
# Modern syntax (gitleaks >= v8.19.0):
gitleaks git --log-opts "origin/main..HEAD" .
# Legacy syntax (gitleaks < v8.19.0):
# gitleaks detect --source . --log-opts "origin/main..HEAD"

# Option 2: git-secrets (fallback if gitleaks not installed)
git secrets --scan

# Option 3: If neither tool is available, run these manual grep patterns:
git diff origin/main..HEAD | grep -E \
  '([A-Za-z0-9+/]{40,}={0,2}|eyJ[A-Za-z0-9+/]+\.[A-Za-z0-9+/]+\.[A-Za-z0-9+/_-]+)'
```

**If any tool reports a finding:**
1. Edit the file to replace the secret with the correct runtime credential access pattern
2. `git add <fixed_file> && git commit --amend --no-edit`
3. Re-run Option 1 (or whichever tool you used above) to confirm clean

**If neither tool is installed and you skip the manual grep:**
Add this warning to the PR description:
```
⚠️ Automated credential scan skipped (gitleaks/git-secrets not installed).
Manual review performed. No credentials found by visual inspection.
```
