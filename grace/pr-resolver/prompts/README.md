...
Markdown prompts consumed by `grace-workspace/packages/core/src/pr-resolver/prompts.ts`. The loader anchors lookup at `cfg.projectRoot + 'grace/pr-resolver/prompts/'`. Each file has YAML frontmatter declaring `name`, `description`, and `variables`. The body is rendered with Mustache; HTML escaping is disabled in the loader so variable values pass through verbatim (cargo output, diff hunks, etc., are not mangled).
...
- Connector-codegen skills (L1–L4) live under `grace-workspace/packages/core/skills/` and are loaded by the Byne pipeline, not the PR resolver. Don't mix the two.
