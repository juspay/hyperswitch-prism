...
/**
 * Build the L2 system prompt with project-rooted absolute paths to the
 * workflow markdown files. Caller must pass the absolute project root
 * (e.g. `cfg.projectRoot` or `task.projectRoot`); falls back to relative
 * paths if `projectRoot` is empty so the prompt is never silently broken.
 */
export function buildL2System(projectRoot: string): string {
  const linksPath = projectRoot
    ? path.join(projectRoot, "grace", "workflow", "2.1_links.md")
    : "grace/workflow/2.1_links.md";
  const techspecPath = projectRoot
    ? path.join(projectRoot, "grace", "workflow", "2.2_techspec.md")
    : "grace/workflow/2.2_techspec.md";
  return L2_SYSTEM_TEMPLATE.replace("{{LINKS_PATH}}", linksPath).replace(
    "{{TECHSPEC_PATH}}",
    techspecPath
  );
}

const L2_SYSTEM_TEMPLATE = `You are a senior payment systems engineer acting as the LINKS AGENT and TECH SPEC AGENT combined.
