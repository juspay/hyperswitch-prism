 ...
 * at `~/.byne/pr-resolver-config.json` (overridable via the second arg to the
 * helpers). Only the safe-to-edit subset of fields is allowed; cargo commands,
 * allow-lists, and path defaults stay file-only.
 */
...
export function defaultRuntimeOverlayPath(): string {
  return path.join(os.homedir(), ".byne", "pr-resolver-config.json");
}
