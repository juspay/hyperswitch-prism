 ...
 * flows (canonical order) first, then core flows (Authorize → PSync →
 * Capture → Refund → RSync → Void). Each subagent assumes prior flows
 * are already implemented.
 */
export function orderedFlowList(detected: DetectedFlows): string[] {
  const orderedPreAuth = PRE_AUTH_FLOWS.filter((f) =>
    detected.preAuth.includes(f)
  );
  return [...orderedPreAuth, ...detected.core];
}
