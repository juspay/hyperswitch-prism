...
# Markers for PazeDecryptedData block
paze_start = 'impl ForeignTryFrom<grpc_api_types::payments::PazeDecryptedData>'
paze_end = 'impl ForeignTryFrom<(Secret<String>, &\'static str)> for SecretSerdeValue'

# Find indices
p_start_idx = content.find(paze_start)
p_end_idx = content.find(paze_end)

if p_start_idx == -1 or p_end_idx == -1:
    print(f"Paze markers not found: start={p_start_idx}, end={p_end_idx}")
    sys.exit(1)

new_paze_block = """impl ForeignTryFrom<grpc_api_types::payments::PazeDecryptedData>
...
# Next marker for end of SecretSerdeValue
secret_serde_end = '// For decoding connector feature data'
s_end_idx = content.find(secret_serde_end)

if s_end_idx == -1:
    print(f"SecretSerde end marker not found")
    sys.exit(1)

new_secret_block = """impl ForeignTryFrom<(Secret<String>, &'static str)> for SecretSerdeValue {
    type Error = ConnectorRequestError;
...
new_content = content[:p_start_idx] + new_paze_block + new_secret_block + content[s_end_idx:]

with open(file_path, 'w') as f:
    f.write(new_content)
print("Successfully reconstructed Paze and SecretSerde blocks.")
