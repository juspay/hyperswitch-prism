...
Extend `grace-workspace/config.yml`:
...
parityAutopilot:
  ...
  cache:
    dir: .cache                 # relative to grace-workspace root
...
**Files:**
- Modify: `grace-workspace/config.yml` (add `parityAutopilot:` block, see schema above)
...
```bash
git add grace-workspace/config.yml packages/parity-core/src/config.ts packages/parity-core/tests/config.test.ts
...
Both use `writeAtomic`. Files written to `$GRACE_WORKSPACE` root (default `grace-workspace/` from config) and `grace-workspace/connectors/`.
...
Steps:
1. `byne parity dashboard` — confirm `grace-workspace/parity-dashboard.md` and `grace-workspace/connectors/<name>.md` files are produced and look right.
