...
RUN --mount=type=cache,target=/sccache \
    cargo chef cook --release --features kafka,connector-request-kafka,otel,log-transformations --recipe-path recipe.json
...
COPY . .
RUN --mount=type=cache,target=/sccache \
    cargo build --release --features kafka,connector-request-kafka,otel,log-transformations -p grpc-server
