...
fn build_client() -> ConnectorClient {
    ...
    let config = ConnectorConfig {
        connector_config: Some(ConnectorSpecificConfig {
            config: Some(connector_specific_config::Config::Paydotcom(
                PaydotcomConfig {
...
pub async fn process_capture(
    ...
    let response = client
        .capture(
            build_capture_request("hld_146231656762572800"),
            &HashMap::new(),
            None,
        )
        .await?;
...
pub async fn process_get(
    ...
    let response = client
        .get(
            build_get_request("chrg_146231656762572800"),
            &HashMap::new(),
            None,
        )
        .await?;
...
pub async fn process_void(
    ...
    let response = client
        .void(
            build_void_request("hld_146231656762572800"),
            &HashMap::new(),
            None,
        )
        .await?;
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_card, process_refund, process_void_payment, process_get_payment, process_authorize, process_capture, process_get, process_pre_authenticate, process_proxy_authorize, process_refund_get, process_void", flow);
            return;
        }
    };
    match result {
