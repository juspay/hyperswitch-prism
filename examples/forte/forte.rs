...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "get", "refund_get", "void"];
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_void_payment, process_get_payment, process_authorize, process_get, process_refund_get, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
