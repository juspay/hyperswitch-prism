...
// Regenerate: python3 scripts/generate-connector-docs.py etisalat
//
// Etisalat — all scenarios and flows in one file.
// Run a scenario:  cargo run --example etisalat -- process_checkout_card
...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "capture", "proxy_authorize", "recurring_charge", "refund", "void"];
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_card, process_refund, process_void_payment, process_authorize, process_capture, process_proxy_authorize, process_recurring_charge, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
