    ...
    let void_response = client
        .void(
            TODO_FIX_MISSING_TYPE_void {
                ...
                cancellation_reason: "requested_by_customer".to_string(),
                // amount: {"minor_amount": 1000, "currency": "USD"}
                connector_transaction_id: Some(authorize_response.connector_transaction_id.clone()), // from Authorize
                ..Default::default()
            },
    ...
    let response = client
        .authenticate(
            TODO_FIX_MISSING_TYPE_authenticate {
                ...
                // redirection_response: {"params": "probe_redirect_params", "payload": {"transaction_id": "probe_txn_123"}}
                ..Default::default()
            },
            &HashMap::new(),
            None,
...
pub async fn process_post_authenticate(
    ...
    let response = client
        .post_authenticate(
            TODO_FIX_MISSING_TYPE_post_authenticate {
                // amount: {"minor_amount": 1000, "currency": "USD"}
                // payment_method: {"card": {"card_number": "4111111111111111", "card_exp_month": "03", "card_exp_year": "2030", "card_cvc": "737", "card_holder_name": "John Doe"}}
                // address: {"billing_address": {}}
                // redirection_response: {"params": "probe_redirect_params", "payload": {"transaction_id": "probe_txn_123"}}
