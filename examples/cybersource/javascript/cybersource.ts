...
// Run a scenario:  npx ts-node cybersource.ts checkout_card

import { PaymentClient, RecurringPaymentClient, PaymentMethodAuthenticationClient } from 'hyperswitch-prism';
...
async function processRecurring(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
    ...
    const setupResponse = await paymentClient.setupRecurring({
        ...
        "metadata": "{\"reference_id\":\"probe_ref_001\",\"connector_request_id\":\"probe_req_001\",\"transaction_id\":\"probe_txn_001\"}",
...
async function preAuthenticate(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
...
export { processCheckoutCard, processCheckoutAutocapture, processCheckoutWallet, processRefund, processRecurring, processVoidPayment, processGetPayment, authorize, capture, get, preAuthenticate, recurringCharge, refund, setupRecurring, voidPayment, _buildPreAuthenticateRequest, _buildRecurringChargeRequest };
