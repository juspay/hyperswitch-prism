...
// Run a scenario:  cargo run --example maya -- process_checkout_card
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_refund, process_void_payment, process_get_payment, process_authorize, process_get, process_parse_event, process_proxy_authorize, process_refund_get, process_token_authorize, process_void", flow);
            return;
        }
    };
    match result {
