...
// Regenerate: python3 scripts/generate-connector-docs.py jpmorgan_orbital
//
// Jpmorgan_Orbital — all scenarios and flows in one file.
// Run a scenario:  cargo run --example jpmorgan_orbital -- process_checkout_card
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_get_payment, process_authorize, process_get, process_proxy_authorize", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
