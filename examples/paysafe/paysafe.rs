...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authenticate", "capture", "customer_create", "get", "pre_authenticate", "refund", "refund_get", "token_authorize", "void"];
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_authenticate, process_capture, process_customer_create, process_get, process_pre_authenticate, process_refund, process_refund_get, process_token_authorize, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
