...
//
// Saferpay — all scenarios and flows in one file.
// Run a scenario:  ./gradlew run --args="saferpay processCheckoutCard"

package examples.saferpay
...
val SUPPORTED_FLOWS = listOf<String>("authorize", "capture", "get", "pre_authenticate", "proxy_setup_recurring", "recurring_charge", "refund_get", "setup_recurring", "void")
...
fun authorize(txnId: String, config: ConnectorConfig = _defaultConfig) {
    ...
    val request = buildAuthorizeRequest("MANUAL")
...
fun main(args: Array<String>) {
    ...
    when (flow) {
        ...
        "void" -> void(txnId)
        else -> System.err.println("Unknown flow: $flow. Available: processCheckoutAutocapture, processCheckoutCard, processVoidPayment, processGetPayment, authorize, capture, get, preAuthenticate, proxySetupRecurring, recurringCharge, refundGet, setupRecurring, void")
    }
}
