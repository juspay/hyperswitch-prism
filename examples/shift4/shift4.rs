...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "capture", "create_client_authentication_token", "customer_create", "get", "incremental_authorization", "proxy_authorize", "proxy_setup_recurring", "recurring_charge", "refund", "refund_get", "setup_recurring", "token_authorize", "token_setup_recurring"];
...
pub fn build_proxy_authorize_request() -> PaymentServiceProxyAuthorizeRequest {
    PaymentServiceProxyAuthorizeRequest {
        ...
        card_proxy: Some(ProxyCardDetails {  // Card proxy for vault-aliased payments (VGS, Basis Theory, Spreedly). Real card values are substituted by the proxy before reaching the connector.
...
pub fn build_proxy_setup_recurring_request() -> PaymentServiceProxySetupRecurringRequest {
    PaymentServiceProxySetupRecurringRequest {
        ...
        card_proxy: Some(ProxyCardDetails {  // Card proxy for vault-aliased payments.
            card_number: Some(Secret::new("4111111111111111".to_string())),  // Card Identification.
            card_exp_month: Some(Secret::new("03".to_string())),
            card_exp_year: Some(Secret::new("2030".to_string())),
            card_cvc: Some(Secret::new("123".to_string())),
        ...
        setup_mandate_details: Some(SetupMandateDetails {
            mandate_type: Some(MandateType {  // Type of mandate (single_use or multi_use) with amount details.
                mandate_type: Some(mandate_type::MandateType::MultiUse(MandateAmountData {
                    amount_money: Some(Money {  // Amount in Money type.
                        minor_amount: 0,  // Amount in minor units (e.g., 1000 = $10.00).
                        currency: Currency::Usd.into(),  // ISO 4217 currency code (e.g., "USD", "EUR").
                    }),
                    ..Default::default()
...
#[allow(dead_code)]
pub async fn process_customer_create(client: &ConnectorClient, _merchant_transaction_id: &str) -> Result<String, Box<dyn std::error::Error>> {
    let response = client.create_customer(build_customer_create_request(), &HashMap::new(), None).await?;
...
#[allow(dead_code)]
pub async fn process_incremental_authorization(client: &ConnectorClient, _merchant_transaction_id: &str) -> Result<String, Box<dyn std::error::Error>> {
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        "process_incremental_authorization" => process_incremental_authorization(&client, "txn_001").await,
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_card, process_refund, process_get_payment, process_authorize, process_capture, process_create_client_authentication_token, process_customer_create, process_get, process_incremental_authorization, process_proxy_authorize, process_proxy_setup_recurring, process_recurring_charge, process_refund_get, process_setup_recurring, process_token_authorize, process_token_setup_recurring", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
