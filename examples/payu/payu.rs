    ...
    let response = client
        .authorize(
            TODO_FIX_MISSING_TYPE_authorize {
                ...
                // address: {"billing_address": {"first_name": "John", "email": "test@example.com", "phone_number": "4155552671", "phone_country_code": "+1"}}
                auth_type: "NO_THREE_DS".to_string(),
                return_url: "https://example.com/return".to_string(),
                // browser_info: {"ip_address": "1.2.3.4"}
                ..Default::default()
            },
            &HashMap::new(),
            None,
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!(
                "Unknown flow: {}. Available: process_authorize, process_get",
                flow
            );
            return;
        }
