...
// Regenerate: python3 scripts/generate-connector-docs.py flywire
//
// Flywire — all scenarios and flows in one file.
// Run a scenario:  cargo run --example flywire -- process_checkout_card
...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "get", "parse_event", "proxy_authorize", "refund", "refund_get", "token_authorize"];
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_refund, process_get_payment, process_authorize, process_get, process_parse_event, process_proxy_authorize, process_refund_get, process_token_authorize", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
