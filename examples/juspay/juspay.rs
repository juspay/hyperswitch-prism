...
// Regenerate: python3 scripts/generate-connector-docs.py juspay
//
// Juspay — all scenarios and flows in one file.
// Run a scenario:  cargo run --example juspay -- process_checkout_card
...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "capture", "create_order", "get", "proxy_authorize", "refund", "refund_get", "void"];
...
pub fn build_authorize_request(capture_method: &str) -> PaymentServiceAuthorizeRequest {
    PaymentServiceAuthorizeRequest {
        ...
        payment_method: Some(PaymentMethod {  // Payment method to be used.
            payment_method: Some(payment_method::PaymentMethod::GooglePaySdk(GoogleWallet {
                r#type: "CARD".to_string(),  // Type of payment method.
                description: "Visa 1111".to_string(),  // User-facing description of the payment method.
                info: Some(PaymentMethodInfo {
                    card_network: "VISA".to_string(),  // Card network name.
                    card_details: "1111".to_string(),  // Card details (usually last 4 digits).
                }),
                tokenization_data: Some(TokenizationData {
                    encrypted_data: Some(GooglePayEncryptedTokenizationData {  // Encrypted Google Pay payment data.
                        token_type: "PAYMENT_GATEWAY".to_string(),  // The type of the token.
                        token: "{\"id\":\"tok_probe_gpay\",\"object\":\"token\",\"type\":\"card\"}".to_string(),  // Token generated for the wallet.
                    }),
                }),
            })),
            ..Default::default()
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_refund, process_void_payment, process_get_payment, process_authorize, process_capture, process_create_order, process_get, process_proxy_authorize, process_refund_get, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
