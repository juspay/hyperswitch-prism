...
// Run a scenario:  cargo run --example square -- process_checkout_card
...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["authorize", "proxy_authorize"];
...
fn build_client() -> ConnectorClient {
    ...
    let config = ConnectorConfig {
        connector_config: Some(ConnectorSpecificConfig {
            config: Some(connector_specific_config::Config::Square(SquareConfig {
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_authorize, process_proxy_authorize", flow);
            return;
        }
    };
    match result {
