...
def _build_refund_request(arg=None):
    ...
    from google.protobuf.json_format import ParseDict
    from payments.generated import payment_pb2
    payload = {'merchant_refund_id': 'probe_refund_001', 'connector_transaction_id': 'probe_connector_txn_001', 'payment_amount': 1000, 'refund_amount': {'minor_amount': 1000, 'currency': 'USD'}, 'reason': 'customer_request', 'state': {'access_token': {'token': {'value': 'probe_key_id|||MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA'}, 'expires_in_seconds': 3600, 'token_type': 'Bearer'}}}
    if arg:
...
def _build_void_request(arg=None):
    ...
    from google.protobuf.json_format import ParseDict
    from payments.generated import payment_pb2
    payload = {'merchant_void_id': 'probe_void_001', 'connector_transaction_id': 'probe_connector_txn_001', 'state': {'access_token': {'token': {'value': 'probe_key_id|||MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA'}, 'expires_in_seconds': 3600, 'token_type': 'Bearer'}}}
    if arg:
        if arg in ('AUTOMATIC', 'MANUAL'):
            payload['capture_method'] = arg
        elif isinstance(arg, str) and 'connector_transaction_id' in payload:
...
def _build_get_request(arg=None):
    ...
    from google.protobuf.json_format import ParseDict
    from payments.generated import payment_pb2
    payload = {'merchant_transaction_id': 'probe_merchant_txn_001', 'connector_transaction_id': 'probe_connector_txn_001', 'amount': {'minor_amount': 1000, 'currency': 'USD'}, 'state': {'access_token': {'token': {'value': 'probe_key_id|||MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA'}, 'expires_in_seconds': 3600, 'token_type': 'Bearer'}}}
    if arg:
        if arg in ('AUTOMATIC', 'MANUAL'):
            payload['capture_method'] = arg
        elif isinstance(arg, str) and 'connector_transaction_id' in payload:
