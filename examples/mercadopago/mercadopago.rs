...
// Run a scenario:  cargo run --example mercadopago -- process_checkout_card
...
fn build_client() -> ConnectorClient {
    ...
    let config = ConnectorConfig {
        connector_config: Some(ConnectorSpecificConfig {
            config: Some(connector_specific_config::Config::Mercadopago(
                MercadopagoConfig {
...
async fn main() {
    ...
    let flow = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "process_create_server_session_authentication_token".to_string());
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!("Unknown flow: {}. Available: process_create_server_session_authentication_token, process_token_authorize", flow);
            return;
        }
    };
    match result {
