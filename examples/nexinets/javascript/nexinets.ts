...
// Run a scenario:  npx ts-node nexinets.ts checkout_autocapture
...
async function processCheckoutWallet(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
    ...
    const authorizeResponse = await paymentClient.authorize({
        ...
        "paymentMethod": {
            "applePay": {
                "paymentData": {
                    "encryptedData": "eyJ2ZXJzaW9uIjoiRUNfdjEiLCJkYXRhIjoicHJvYmUiLCJzaWduYXR1cmUiOiJwcm9iZSJ9"
                },
                "paymentMethod": {
                ...
                "transactionIdentifier": "probe_txn_id"
            }
        },
        "captureMethod": "AUTOMATIC",
        "address": {
