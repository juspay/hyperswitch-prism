...
// Run a scenario:  npx ts-node stripe.ts checkout_card

import { PaymentClient, RecurringPaymentClient, CustomerClient, PaymentMethodClient } from 'hyperswitch-prism';
...
async function processCreateCustomer(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
...
async function createCustomer(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
    const customerClient = new CustomerClient(config);
...
export { processCheckoutCard, processCheckoutAutocapture, processCheckoutWallet, processCheckoutBank, processRefund, processRecurring, processVoidPayment, processGetPayment, processCreateCustomer, processTokenize, authorize, capture, createCustomer, get, recurringCharge, refund, setupRecurring, tokenize, voidPayment, _buildCreateCustomerRequest, _buildRecurringChargeRequest, _buildTokenizeRequest };
