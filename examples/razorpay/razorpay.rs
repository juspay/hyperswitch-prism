    ...
    let authorize_response = client
        .authorize(
            TODO_FIX_MISSING_TYPE_authorize {
                ...
                // address: {"billing_address": {"phone_number": "4155552671"}}
                auth_type: "NO_THREE_DS".to_string(),
                return_url: "https://example.com/return".to_string(),
                connector_order_id: "connector_order_id".to_string(),
                ..Default::default()
            },
            &HashMap::new(),
            None,
    ...
    let response = client
        .parse_event(
            TODO_FIX_MISSING_TYPE_parse_event {
                // request_details: {"method": "HTTP_METHOD_POST", "uri": "https://example.com/webhook", "headers": {}, "body": "{\"account_id\":\"probe_acct\",\"contains\":[\"payment\"],\"entity\":\"event\",\"event\":\"payment.captured\",\"payload\":{\"payment\":{\"entity\":{\"id\":\"pay_probe001\",\"entity\":\"payment\",\"amount\":1000,\"currency\":\"USD\",\"status\":\"captured\",\"order_id\":\"order_probe001\"}}}}"}
                ..Default::default()
            },
            &HashMap::new(),
            None,
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => {
            eprintln!("Unknown flow: {}. Available: process_checkout_autocapture, process_checkout_card, process_refund, process_get_payment, process_authorize, process_capture, process_create_order, process_get, process_parse_event, process_proxy_authorize, process_refund_get", flow);
            return;
        }
    };
    match result {
