...
// Run a scenario:  npx ts-node adyen.ts checkout_card

import { PaymentClient, RecurringPaymentClient, DisputeClient } from 'hyperswitch-prism';
...
async function disputeAccept(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
...
async function disputeDefend(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
    const disputeClient = new DisputeClient(config);
...
async function disputeSubmitEvidence(merchantTransactionId: string, config: types.IConnectorConfig = _defaultConfig): Promise<unknown> {
    const disputeClient = new DisputeClient(config);
