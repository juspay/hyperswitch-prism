...
#[allow(dead_code)]
pub const SUPPORTED_FLOWS: &[&str] = &["capture", "get", "pre_authenticate", "recurring_charge", "refund", "refund_get", "setup_recurring", "void"];
...
async fn main() {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = match flow.as_str() {
        ...
        _ => { eprintln!("Unknown flow: {}. Available: process_capture, process_get, process_pre_authenticate, process_recurring_charge, process_refund, process_refund_get, process_setup_recurring, process_void", flow); return; }
    };
    match result {
        Ok(msg) => println!("✓ {msg}"),
        Err(e) => eprintln!("✗ {e}"),
