...
fn map_app_err_to_connector_req<T>(
    result: Result<T, error_stack::Report<ApplicationErrorResponse>>,
) -> Result<T, error_stack::Report<ConnectorRequestError>> {
    result.map_err(|e| {
        e.change_context(ConnectorRequestError::InvalidDataFormat { field_name: "unknown" })
    })
}
...
use crate::{
    ...
    errors::{
        ApplicationErrorResponse, ConnectorRequestError, ConnectorResponseError,
    },
                ...
                grpc_api_types::payments::payment_method::PaymentMethod::ApplePay(apple_wallet) => {
                    ...
                    let applepay_payment_data = match payment_data.payment_data {
                                ...
                                Some(grpc_api_types::payments::apple_wallet::payment_data::PaymentData::DecryptedData(decrypted_data)) => {
                                    let payment_data = map_app_err_to_connector_req(
                                    ...
                                    Ok(payment_method_data::ApplePayPaymentData::Decrypted(
                                        payment_method_data::ApplePayDecryptedData {
                                            application_primary_account_number: map_app_err_to_connector_req(
                                                payment_method_data::ApplePayWalletData::validate_decrypted_primary_account_number(
                                                    decrypted_data.application_primary_account_number,
                                                ),
                                            )?,
                                            application_expiration_month: map_app_err_to_connector_req(
                                                payment_method_data::ApplePayWalletData::validate_decrypted_expiration_month(
                                                    decrypted_data.application_expiration_month,
                                                ),
                                            )?,
                                            application_expiration_year: map_app_err_to_connector_req(
                                                payment_method_data::ApplePayWalletData::validate_decrypted_expiration_year(
    ...
    > ForeignTryFrom<PaymentServiceAuthorizeRequest> for PaymentsAuthorizeData<T>
{
    ...
    fn foreign_try_from(
        ...
        let authentication_data = value
            .authentication_data
            .clone()
            .map(|v| map_app_err_to_connector_req(router_request_types::AuthenticationData::try_from(v)))
            .transpose()?;
