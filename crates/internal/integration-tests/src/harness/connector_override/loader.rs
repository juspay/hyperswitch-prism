...
mod tests {
    ...
    fn an_invented_field_is_rejected_rather_than_ignored() {
        // Before `deny_unknown_fields`, this parsed cleanly and every field came
        // back None: the diff read as a declaration while the run ignored it.
        // "unsupported"/"reason" is the exact pair people reach for, and the
        // silence is worse than an error because nothing points at it.
        let err = serde_json::from_value::<ScenarioOverridePatch>(json!({
            "unsupported": true,
            "reason": "connector does not support UPI"
        }))
