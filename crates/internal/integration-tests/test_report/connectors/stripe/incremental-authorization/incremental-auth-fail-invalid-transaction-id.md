# Connector `stripe` / Suite `incremental_authorization` / Scenario `Incremental Authorization | Incremental Auth Fail Invalid Transaction ID`

- Service: `Unknown`
- Scenario Key: `incremental_auth_fail_invalid_transaction_id`
- PM / PMT: `-` / `-`
- Result: `FAIL`
...
```text
assertion failed for field 'status': expected one of ["FAILED", "PROCESSING_ERROR"], got "AUTHORIZATION_FAILURE"
```
...
grpcurl -plaintext \
  -H "x-merchant-id: test_merchant" \
  -H "x-tenant-id: default" \
  -H "x-request-id: incremental_authorization_incremental_auth_fail_invalid_transaction_id_req" \
  -H "x-connector-request-reference-id: incremental_authorization_incremental_auth_fail_invalid_transaction_id_ref" \
  -H "x-connector-config: ***MASKED***" \
...
Request metadata to send:
x-connector-config: ***MASKED***
x-connector-request-reference-id: incremental_authorization_incremental_auth_fail_invalid_transaction_id_ref
x-merchant-id: test_merchant
x-request-id: incremental_authorization_incremental_auth_fail_invalid_transaction_id_req
x-tenant-id: default
...
Response contents:
{
  ...
  "responseHeaders": {
    ...
    "content-type": "application/json",
    "date": "Mon, 23 Mar 2026 15:48:25 GMT",
    "idempotency-key": "fa29878f-752d-43e5-8d41-65ffeb41ce6b",
    "original-request": "req_IoQtWqMHigollw",
    "request-id": "req_IoQtWqMHigollw",
    "server": "nginx",
    "strict-transport-security": "max-age=63072000; includeSubDomains; preload",
    "stripe-version": "2022-11-15",
    "vary": "Origin",
