# Connector `worldpay` / Suite `authorize`
...
- [`Google Pay (Encrypted Token) | No 3DS | Automatic Capture`](./authorize/no3ds-auto-capture-google-pay-encrypted.md) — grpcurl execution failed: [google_pay_token_gen] worldpay/authorize/no3ds_auto_capture_google_pay_encrypted: npm exited with exit status: 1
...
- [`Payment Failure | No 3DS`](./authorize/no3ds-fail-payment.md) — assertion failed for field 'error.connector_details.message': expected 'Invalid Configuration: connector_config.merchant_name' to contain 'decline'
