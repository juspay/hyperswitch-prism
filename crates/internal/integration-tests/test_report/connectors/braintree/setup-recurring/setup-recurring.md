# Connector `braintree` / Suite `setup_recurring` / Scenario `setup_recurring`
...
```text
sdk call failed: sdk request transformer failed for 'setup_recurring/setup_recurring': Failed to obtain authentication type (code: INTERNAL_SERVER_ERROR)
```
