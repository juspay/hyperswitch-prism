# Connector `nexixpay` / Suite `get` / Scenario `Get | Sync Payment With Handle Response`
...
```text
sdk call failed: sdk request transformer failed for 'authorize/no3ds_auto_capture_credit_card': Missing required field: authentication_data (must be present for 3DS flow) (code: BAD_REQUEST)
```
