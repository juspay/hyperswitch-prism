...
impl ForeignTryFrom<grpc_api_types::payments::ConnectorSpecificConfig> for ConnectorSpecificConfig {
    ...
    fn foreign_try_from(
        ...
        match auth_type {
            ...
            AuthType::Tpay(tpay) => Ok(Self::Tpay {
                api_key: tpay.api_key.ok_or_else(err)?,
                client_secret: tpay.client_secret.ok_or_else(err)?,
                base_url: tpay.base_url,
            }),
    ...
    fn foreign_try_from(
        ...
        match connector {
            ConnectorVariant::Payment(connector_enum) => match connector_enum {
                ...
                ConnectorEnum::Tpay => match auth {
                ConnectorAuthType::BodyKey { api_key, key1 } => Ok(Self::Tpay {
                    api_key: api_key.clone(),
                    client_secret: key1.clone(),
                    base_url: None,
                }),
