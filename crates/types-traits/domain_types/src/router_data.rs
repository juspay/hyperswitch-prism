...
impl ForeignTryFrom<grpc_api_types::payments::ConnectorSpecificConfig> for ConnectorSpecificConfig {
    ...
    fn foreign_try_from(
        ...
        match auth_type {
            ...
            AuthType::Btcpay(btcpay) => Ok(Self::Btcpay {
                api_key: btcpay.api_key.ok_or_else(err)?,
                store_id: btcpay.store_id.map(Secret::new),
                base_url: btcpay.base_url,
            }),
    ...
    fn foreign_try_from(
        ...
        match connector {
            ConnectorVariant::Payment(connector_enum) => match connector_enum {
                ...
                ConnectorEnum::Btcpay => match auth {
                    ConnectorAuthType::HeaderKey { api_key } => Ok(Self::Btcpay {
                        api_key: api_key.clone(),
                        store_id: None,
                        base_url: None,
                    }),
