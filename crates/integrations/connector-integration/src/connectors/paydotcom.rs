...
//!
//! Flows: Authorize, PSync, Capture, Void, Refund, RSync, plus PreAuthenticate and
//! Authenticate for the gateway-driven 3DS challenge journey.
//!
//! # Endpoint map
//!
//! | Flow | Method | Path |
...
//! | PreAuthenticate (3DS leg 1) | POST | `/v1/charges` or `/v1/holds` |
//! | Authenticate (3DS leg 2) | POST | `/v1/sessions/authentication/linked` |
//! | Authorize (3DS leg 3) | POST | `/v1/{charges\|holds}/{id}/confirm` |
//!
//! # 3-D Secure
//!
...
//!   split across the three authentication flows, as Paysafe and Redsys do:
//!
...
//!   | 2 | `Authenticate` | `POST /v1/sessions/authentication/linked` `{resource, return_url, confirm:false}` | the challenge `url` → `RedirectForm` |
//!   | 3 | `Authorize` | `POST /v1/{charges\|holds}/{id}/confirm` | the final status, read synchronously |
//!
//!   The resource id travels between legs on **`authentication_data.transaction_id`**
//!   (wire field 8, `connector_transaction_id`, documented as "transaction identifier
//!   generated by the 3DS system" — which is what this id is here). That is the channel an
//!   orchestrator already carries for free: Hyperswitch lifts the PreAuthenticate
//!   response's `authentication_data` into the Authenticate request, persists the
//!   Authenticate response's copy on the attempt, and replays it on the settling
//!   CompleteAuthorize. No connector-specific metadata plumbing is required anywhere.
//!
//!   `transformers::resource_id_authentication_data` fills in only `transaction_id`,
//!   deliberately: an `eci`/`cavv` there would be indistinguishable from a
//!   merchant-supplied external-MPI authentication (`transformers::three_ds_mode`), and
//!   Pay.com hands the merchant neither on this journey.
//!
//!   The id is *mirrored* on `PaymentFlowData::connector_feature_data` as
//!   `{"paydotcom_resource": "<id>"}` for callers that drive the gRPC flows directly
//!   (grpcurl, the SDKs, the composite service) rather than through an orchestrator that
//!   moves `authentication_data` for them. Both channels are read back
//!   (`transformers::authorize_pending_resource_id`).
//!
...
//!
//!   The Authorize leg is chosen from the **request alone**
...
use serde::Serialize;
use transformers::{
    self as paydotcom, PaydotcomAuthenticateRequest, PaydotcomAuthenticateResponse,
...
// Authenticate — gateway-3DS leg 2. Turns the resource PreAuthenticate parked on
// `requires_authentication` into a challenge session and hands back its URL.
macros::macro_connector_implementation!(
    connector_default_implementations: [get_content_type, get_error_response_v2],
    connector: Paydotcom,
    curl_request: Json(PaydotcomAuthenticateRequest),
    curl_response: PaydotcomAuthenticateResponse,
...
impl<T: PaymentMethodDataTypes + Debug + Sync + Send + 'static + Serialize>
    connector_types::PaymentAuthenticateV2<T> for Paydotcom<T>
{
}
