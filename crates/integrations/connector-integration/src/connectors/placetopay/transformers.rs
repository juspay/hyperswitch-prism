...
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct PlacetopayVoidPCResponse(pub PlacetopayPaymentsResponse);
    ...
    for RouterDataV2<F, PaymentFlowData, PaymentsCancelPostCaptureData, PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        let post_capture_void_status = match item.response.0.status.status {
            PlacetopayTransactionStatus::Approved | PlacetopayTransactionStatus::Ok => {
        ...
        Ok(Self {
            response: Ok(PaymentsResponseData::PostCaptureVoidResponse {
                ...
                connector_reference_id: Some(item.response.0.internal_reference.to_string()),
                description: None,
                status_code: item.http_code,
            }),
            ..item.router_data
