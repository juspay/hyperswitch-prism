...
mod tests {
    ...
    /// ```
    ///
    /// then record the vectors in `REFERENCE_VECTORS` below so they become
    /// part of the normal suite (only the digested expectations are stored —
    ...
    fn kount_reference_vectors() {
        ...
        // TODO(Kount reference vectors): replace with the six official
        // `<number, expected token>` pairs once Kount delivers them.
        let reference_vectors: &[(&str, &str)] = &[];
        assert!(
            !reference_vectors.is_empty(),
            "no Kount reference vectors recorded yet"
        );
        for (number, expected) in reference_vectors {
            assert_eq!(&khash.hash(number, CARD_TOKEN_SUFFIX_LENGTH), expected);
        }
    }
}
