...
mod tests {
    ...
    fn card_number(pan: &str) -> CardNumber {
        CardNumber::from_str(pan).expect("static test PAN")
    }

    fn card_pm_card_type(
        card_type: Option<&str>,
        pan: &str,
    ) -> PaymentMethodData<DefaultPCIHolder> {
        let card = Card::<DefaultPCIHolder> {
            card_number: RawCardNumber(card_number(pan)),
    ...
    #[test]
    fn card_with_number_is_khash_card_shaped() {
        let pm = card_pm_card_type(Some("credit"), "4111111111111111");
        let instrument = kount_instrument(&pm, None).unwrap();
        assert_eq!(instrument.payment_type, KountPaymentType::CreditCard);
        assert_eq!(instrument.hash_kind, TokenHashKind::Khash);
        assert!(instrument.card_shaped);
        assert_eq!(instrument.token_source.as_deref(), Some("4111111111111111"));
    }

    #[test]
    fn unknown_card_type_falls_back_to_credit_card() {
        let pm = card_pm_card_type(None, "4111111111111111");
        let instrument = kount_instrument(&pm, None).unwrap();
        assert_eq!(instrument.payment_type, KountPaymentType::CreditCard);
    }

    #[test]
    fn card_debit_type_is_preserved() {
        let pm = card_pm_card_type(Some("debit"), "4111111111111111");
        let instrument = kount_instrument(&pm, None).unwrap();
        assert_eq!(instrument.payment_type, KountPaymentType::DebitCard);
    }
}
