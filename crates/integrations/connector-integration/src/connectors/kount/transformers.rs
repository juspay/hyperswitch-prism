        ...
        let payment = req.payment_method.as_ref().and_then(|pm| {
            kount_instrument(pm, req.payment_method_type).map(
                |instrument| -> Result<KountPayment, errors::IntegrationError> {
                    ...
                    let payment_token =
                        match (instrument.token_source.as_deref(), instrument.hash_kind) {
                            (Some(source), TokenHashKind::Khash) => match &khash {
                                Some(khash) => Some(
                                    khash.hash(source, super::khash::CARD_TOKEN_SUFFIX_LENGTH),
                                ),
                                None => {
                                    tracing::warn!(
                                        connector = "kount",
                    ...
                    // Expiry is only defined for cards; Kount has no equivalent
                    // for wallets/bank transfers/etc.
                    let (exp_month, exp_year) = match pm {
                        PaymentMethodData::Card(card) => card_expiry(card)?,
                        _ => (None, None),
                    };
                    Ok(KountPayment {
