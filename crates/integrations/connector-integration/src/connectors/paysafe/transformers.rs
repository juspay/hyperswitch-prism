...
mod skip_3ds_tests {
    use domain_types::payment_method_data::{
        DefaultPCIHolder, GooglePayPaymentMethodInfo, GooglePayWalletData,
        GpayEncryptedTokenizationData, RawCardNumber,
    };

    use super::*;

    fn decrypted(cryptogram: Option<&str>) -> GooglePayDecryptedData {
        GooglePayDecryptedData {
            card_exp_month: Secret::new("12".to_string()),
            card_exp_year: Secret::new("2030".to_string()),
            application_primary_account_number: "4111111111111111".to_string().try_into().unwrap(),
            cryptogram: cryptogram.map(|c| Secret::new(c.to_string())),
            eci_indicator: Some("05".to_string()),
        }
    }

    fn google_pay(tokenization_data: GpayTokenizationData) -> PaymentMethodData<DefaultPCIHolder> {
        PaymentMethodData::Wallet(WalletData::GooglePay(GooglePayWalletData {
    ...
    /// The reported bug: a PAN_ONLY token carries no authentication, so a THREE_D_S_TWO
    /// account rejects the handle with 5068 unless `skip3ds` is sent.
    #[test]
    ...
    /// stay off rather than assert 3DS was skipped.
    #[test]
    ...
    /// existing wire shape untouched.
    #[test]
    fn google_pay_encrypted_omits_skip_3ds() {
        let pmd = google_pay(GpayTokenizationData::Encrypted(
    ...
    /// (`tokenWith3DS` vs `panOnlyToken`).
    #[test]
    fn auth_method_tracks_cryptogram_presence() {
        let with = decrypted(Some("AgAAAA=="));
        let without = decrypted(None);
        assert!(with.cryptogram.is_some());
        assert!(without.cryptogram.is_none());
        // Guards the enum wire values Paysafe matches on.
        assert_eq!(
            serde_json::to_string(&PaysafeGooglePayAuthMethod::Cryptogram3Ds).unwrap(),
            "\"CRYPTOGRAM_3DS\""
        );
        assert_eq!(
            serde_json::to_string(&PaysafeGooglePayAuthMethod::PanOnly).unwrap(),
            "\"PAN_ONLY\""
        );
    }

    /// The wire key must be exactly `skip3ds`. `rename_all = "camelCase"` would turn the
    /// Rust field `skip_3ds` into `skip3Ds`, which Paysafe ignores — the handle would then
    /// still 5068 with no visible cause.
    #[test]
    fn skip_3ds_serializes_with_the_exact_paysafe_key() {
        let req = PaysafeSetupMandateRequest::<DefaultPCIHolder> {
            merchant_ref_num: "ref".to_string(),
            amount: common_utils::types::MinorUnit::new(100),
            settle_with_auth: Some(true),
            payment_method: PaysafePaymentMethod::Card {
                card: PaysafeCard {
                    card_num: domain_types::payment_method_data::RawCardNumber(
                        "4111111111111111".to_string().try_into().unwrap(),
                    ),
                    card_expiry: PaysafeCardExpiry {
            ...
            currency_code: common_enums::Currency::EUR,
            payment_type: PaysafePaymentType::Card,
            transaction_type: TransactionType::Payment,
            return_links: None,
            account_id: None,
