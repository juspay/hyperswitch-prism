...
use domain_types::{
    connector_flow::{Authorize, Capture, CreateAccessToken, CreateOrder, RSync, Refund},
    connector_types::{
                ...
                let order_id =
                    response
                        ...
                        .ok_or(ConnectorError::MissingRequiredField {
                            field_name: "order_id in CreateOrder response",
                        })?;
...
fn build_payment_option<T: PaymentMethodDataTypes>(
    payment_method_data: &PaymentMethodData<T>,
) -> Result<PinelabsOnlinePaymentOption, ConnectorError> {
    match payment_method_data {
...
fn build_card_details<T: PaymentMethodDataTypes>(
    card: &Card<T>,
) -> Result<PinelabsOnlineCardDetails, ConnectorError> {
    let name = card
        .card_holder_name
        .as_ref()
